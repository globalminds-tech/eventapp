...
from sqlalchemy import String, Text, Boolean, Integer, Date, Time, DateTime, ForeignKey, Numeric, func, Uuid
...
class EventDetails(db.Model):
    ...
    user_id: Mapped[Optional[uuid_pkg.UUID]] = mapped_column(Uuid, ForeignKey('users.id', ondelete='SET NULL'), nullable=True)
    organization_id: Mapped[Optional[uuid_pkg.UUID]] = mapped_column(Uuid, nullable=True, index=True)
...
class EventTerm(db.Model):
    ...
    id: Mapped[uuid_pkg.UUID] = mapped_column(Uuid, primary_key=True, default=uuid_pkg.uuid4)
    event_id: Mapped[uuid_pkg.UUID] = mapped_column(Uuid, ForeignKey('event_details_table.id', ondelete='CASCADE'), nullable=False)
