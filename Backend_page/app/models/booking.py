...
from sqlalchemy import String, Text, Boolean, ForeignKey, DateTime, Integer, Numeric, func, Uuid
...
class AttendeeCheckinLog(db.Model):
    ...
    id: Mapped[uuid_pkg.UUID] = mapped_column(Uuid, primary_key=True, default=uuid_pkg.uuid4)
    booking_id: Mapped[uuid_pkg.UUID] = mapped_column(Uuid, ForeignKey('user_booking_details.id', ondelete='CASCADE'), nullable=False, index=True)
    ticket_code: Mapped[Optional[str]] = mapped_column(String(60), nullable=True)
    event_id: Mapped[uuid_pkg.UUID] = mapped_column(Uuid, nullable=False, index=True)
