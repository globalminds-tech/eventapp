...
class StallRepository:
    @staticmethod
    def get_stalls_by_event(event_id: int) -> list[EventStall]:
        stmt = select(EventStall).where(EventStall.event_id == event_id)
        return list(db.session.scalars(stmt).all())

    @staticmethod
    def get_amenities_by_event(event_id: int) -> list[StallAmenity]:
        stmt = select(StallAmenity).where(StallAmenity.event_id == event_id)
        return list(db.session.scalars(stmt).all())

    @staticmethod
    def get_by_id(stall_id: int) -> EventStall | None:
